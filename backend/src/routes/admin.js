...
router.get('/users', async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT id, email, name, phone, is_admin, role, created_at 
      FROM users 
...
router.put('/users/:id/admin', async (req, res) => {
  try {
    ...
    const role = is_admin ? 'superuser' : 'customer';
...
// Update a user's dashboard role. This lets the superuser create a lower-access
// product assistant without granting pricing, orders, users, or settings access.
router.put('/users/:id/role', async (req, res) => {
  try {
    const { id } = req.params;
    const requestedRole = String(req.body?.role || 'customer').trim().toLowerCase();
    const allowedRoles = new Set(['customer', 'product_assistant', 'superuser']);
    ...
    const result = await pool.query(`
      ...
      WHERE id = $3
      RETURNING id, email, name, is_admin, role
    `, [requestedRole, requestedRole === 'superuser', id]);
